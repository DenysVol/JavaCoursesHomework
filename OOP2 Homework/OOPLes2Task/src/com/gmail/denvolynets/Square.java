...
public class Square extends Shape {
	...
	public Square(Point pOne, Point pTwo, Point pThree, Point pFour) {
		super();
		this.pOne = pOne;
		this.pTwo = pTwo;
		this.pThree = pThree;
	...
	double getPerimetr() {
		double p = 0;
		if (makeSide(pOne, pTwo) != makeSide(pTwo, pThree) 
				|| makeSide(pTwo, pThree) != makeSide(pThree, pFour)
				|| makeSide(pThree, pFour) != makeSide(pFour, pOne)
	...
	double getArea() {
		...
		if (getPerimetr() < 0) {
			s = -1;
			System.out.println("Square doesn't exists");
		} else {
			s = Math.pow(getPerimetr() / 4, 2);
		}
		return s;
	}
